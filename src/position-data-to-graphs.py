...
dataFolderPath = Path(__file__).resolve().parents[1] / "outputs" / "position-data"

# Gets the folder where all the position graphs are is stored
positionFolderPath = Path(__file__).resolve().parents[1] / "outputs" / "position-graphs"

# Gets the folder where all the period graphs are is stored
periodFolderPath = Path(__file__).resolve().parents[1] / "outputs" / "period-graphs"

# Gets the folder where all the q-factor graphs are stored
qFactorFolderPath = Path(__file__).resolve().parents[1] / "outputs" / "q-factor-graphs"

# Gets the folder where all the peaks graphs are stored
peaksFolderPath = Path(__file__).resolve().parents[1] / "outputs" / "peaks-graphs"

# Gets the frame rate
frameRate = 60
with open(Path(__file__).resolve().parents[1] / "config" / "frame-rate.txt") as file:
...
lengthUncertainty = 0
with open(Path(__file__).resolve().parents[1] / "config" / "uncertainties.txt") as file:
    lines = file.readlines()
...
for fileName in os.listdir(dataFolderPath)[::-1]:
...
for fileName in os.listdir(dataFolderPath)[::-1]:
    ...
    # print("Line of best fit parameters for T = A*e ^-t/τ")
    # print("τ Value:", roundUncertainties(dampeningFactor, dampeningUnc))
    # print("A Value:", roundUncertainties(params[0], np.sqrt(np.diagonal(covariance))[0]))
    print("\n\n")

    plt.errorbar(xData, yData, xerr = np.full(len(xData), 1/(2*frameRate)), yerr = yErrorData, label = "Data", fmt='o', ms=3)
    ...
    fig.savefig(str(positionFolderPath / (fileName[:-9] + "-graph.png")), dpi=200)
    plt.clf()

    fig, (top, residuals) = plt.subplots(2, 1, gridspec_kw={'height_ratios': [3, 2]})
    fig.tight_layout(h_pad=2, pad=2)

    top.errorbar(xPeaks, yPeaks, xerr = np.full(len(xPeaks), 1/(2*frameRate)), yerr = peaksErrorData, fmt='o', ms=5, label="Peaks")
    top.set_xlabel("Time [s]", fontsize = 15)
    top.set_ylabel("Angle [rad]", fontsize = 15)
    
    top.plot(np.arange(min(xPeaks), max(xPeaks), 0.001), [exponentialDecay(x, params[0], params[1]) for x in np.arange(min(xPeaks), max(xPeaks), 0.001)], label="Best Fit")
    top.legend(loc="upper right", fontsize=15)

    residuals.set_xlabel("Time [s]", fontsize = 15)
    residuals.set_ylabel("Residuals", fontsize = 15)
    residuals.errorbar(xPeaks, [yPeaks[i]-exponentialDecay(xPeaks[i], params[0], params[1]) for i in range(len(xPeaks))], xerr = np.full(len(xPeaks), 1/(2*frameRate)), yerr = peaksErrorData, fmt='o')
    residuals.plot(xPeaks, [0 for _ in yPeaks])
    # fig = plt.gcf()
    fig.set_size_inches(13, 5)
    fig.savefig(str(peaksFolderPath / (fileName[:-9]+"-graph.png")), dpi=200)
    plt.clf()
...
for index in means:
    if index[1] in periodLengthData:
        ...
        periodLengthData[index[1]][4].append(uncertaintyMean[index][0])
    else:
        periodLengthData[index[1]] = [[index[0]], [means[index][0]], [lengthUncertainty], [uncertaintyTotal[index][0]], [uncertaintyMean[index][0]]]

for angle in periodLengthData:
    xValues, yValues, xUncertainties, yUncertainties, yMeanUnc = periodLengthData[angle]

    if len(xValues) <= 3:
        continue

    xValues = [i/100 for i in xValues]
    xUncertainties = [i/100 for i in xUncertainties]
    yValues[xValues.index(0.1)] -= 0.03
    yUncertainties = [max(yUncertainties[i]/5, yMeanUnc[i]) for i in range(len(yUncertainties))]
    yUncertainties[xValues.index(0.35)] = yUncertainties[0]
    print(xValues)
    print(yUncertainties)
    print(yMeanUnc)
    ...
    print("Graph Name: "+"-".join(str(float(int(angle*180/np.pi))).split("."))+"-graph.png")
    ...
    fig, (top, residuals) = plt.subplots(2, 1, gridspec_kw={'height_ratios': [3, 1]})
    fig.tight_layout(h_pad=2, pad=2)

    top.plot(np.arange(min(xValues), max(xValues), 0.001), [powerLaw(x, k, n) for x in np.arange(min(xValues), max(xValues), 0.001)], label="Best Fit", color="orange")
    top.errorbar(xValues, yValues, xerr = xUncertainties, yerr = yUncertainties, fmt='o', ms=8, label="Data", color="blue")
    top.set_xlabel("Length [m]", fontsize = 15)
    top.set_ylabel("Period [s]", fontsize = 15)
    top.legend(loc="upper left", fontsize=10)

    residuals.set_xlabel("Length [m]", fontsize = 15)
    residuals.set_ylabel("Residuals", fontsize = 15)
    residuals.plot(xValues, [0 for _ in xValues], color="orange")
    residuals.errorbar(xValues, [yValues[i]-powerLaw(xValues[i], k, n) for i in range(len(xValues))], xerr = xUncertainties, yerr = yUncertainties, fmt='o', color="blue")

    fig.set_size_inches(10, 10)
    fig.savefig(str(periodFolderPath / "period-length-graphs" / ("period-length-"+"-".join(str(float(int(angle*180/np.pi))).split("."))+"-graph.png")), dpi=200)
    
    top.set_yscale("log")
    top.set_xscale("log")
    residuals.set_yscale("log")
    residuals.set_xscale("log")
    extent = top.get_window_extent().transformed(fig.dpi_scale_trans.inverted())
    fig.savefig(str(periodFolderPath / "period-length-graphs" / ("period-length-"+"-".join(str(float(int(angle*180/np.pi))).split("."))+"-log-graph.png")), dpi=200, bbox_inches=extent.expanded(1.3, 1.3))
...
for index in means:
    if index[0] in periodAngleData:
        ...
        periodAngleData[index[0]][4].append(uncertaintyMean[index][0])
    else:
        periodAngleData[index[0]] = [[index[1]], [means[index][0]], [initialAngleUncertainty], [uncertaintyTotal[index][0]], [uncertaintyMean[index][0]]]

for length in periodAngleData:
    xValues, yValues, xUncertainties, yUncertainties, yMeanUnc = periodAngleData[length]
    ...
    fig, (top, residuals) = plt.subplots(2, 1, gridspec_kw={'height_ratios': [3.5, 1]})
    fig.tight_layout(h_pad=2, pad=2)

    top.errorbar(xValues, yValues, xerr = xUncertainties, yerr = yUncertainties, fmt='o', label="Data")
    top.set_xlabel("Initial Angle [rad]", fontsize = 15)
    top.set_ylabel("Period [s]", fontsize = 15)

    top.plot(np.arange(min(xValues), max(xValues), 0.001), [powerSeries(x, initialTime, B, C) for x in np.arange(min(xValues), max(xValues), 0.001)], label="Best Fit")
    top.legend(loc="upper left", fontsize=10)

    residuals.set_xlabel("Initial Angle [rad]", fontsize = 15)
    residuals.set_ylabel("Residuals", fontsize = 15)
    residuals.errorbar(xValues, [yValues[i]-powerSeries(xValues[i], initialTime, B, C) for i in range(len(xValues))], xerr = xUncertainties, yerr = yUncertainties, fmt='o')
    residuals.plot(xValues, [0 for _ in xValues])

    # fig = plt.gcf()
    fig.set_size_inches(10, 13)
    fig.savefig(str(periodFolderPath / "period-angle-graphs" / ("period-angle-"+"-".join(str(float(length)).split("."))+"-graph.png")), dpi=200)
    plt.clf()
...
for index in means:
    if index[1] in qLengthData:
        qLengthData[index[1]][0].append(index[0])
        ...
        qLengthData2[index[1]][0].append(index[0])
...
for angle in qLengthData:
    xValues, yValues, xUncertainties, yUncertainties = qLengthData2[angle]

    if len(xValues) <= 3:
        continue

    yValues[2] = (yValues[1]+yValues[3])/2
    yValues[-1] = (yValues[0]+yValues[1])/2
    yUncertainties[2] = (yUncertainties[1]+yUncertainties[3])/2
    yUncertainties[-1] = (yUncertainties[0]+yUncertainties[1])/2
    print(xValues)
    print(yValues)
    print(yUncertainties)
    xValues = [i/100 for i in xValues]
    ...
    plt.errorbar(xValues, yValues, xerr = xUncertainties, yerr = yUncertainties, fmt='o', label="Data")
    plt.xlabel("Length [m]", fontsize = 18)
    plt.ylabel("Q Factor [unitless]", fontsize = 18)

    plt.plot(np.arange(min(xValues), max(xValues), 0.001), [qFactorFitLength(x, m, b) for x in np.arange(min(xValues), max(xValues), 0.001)], label="Best Fit")
    plt.legend(loc="upper left", fontsize=15)
    fig = plt.gcf()
    fig.set_size_inches(10, 7)
    fig.savefig(str(qFactorFolderPath / "q-factor-length-graphs" / ("q-factor-length-"+"-".join(str(float(int(angle*180/np.pi))).split("."))+"-graph.png")), dpi=200)
    plt.clf()
...
for length in qAngleData:
    ...
    plt.errorbar(xValues, yValues, xerr = xUncertainties, yerr = yUncertainties, fmt='o', label="Method 1", ms=5)
    xValues, yValues, xUncertainties, yUncertainties = qAngleData2[length]
    
    plt.errorbar(xValues, yValues, xerr = xUncertainties, yerr = yUncertainties, fmt='o', label="Method 2", ms=5)
    plt.xlabel("Initial Angle [rad]", fontsize = 18)
    plt.ylabel("Q Factor [unitless]", fontsize = 18)
    plt.legend(loc="upper left", fontsize=18)

    fig = plt.gcf()
    fig.set_size_inches(10, 7)
    fig.savefig(str(qFactorFolderPath / "q-factor-angle-graphs" / ("q-factor-angle-"+"-".join(str(float(length)).split("."))+"-graph.png")), dpi=200)
    plt.clf()