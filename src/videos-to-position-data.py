...
videoFolderPath = Path(__file__).resolve().parents[1] / "inputs"
...
lowerBoundsMass = []
upperBoundsMass = []
with open(Path(__file__).resolve().parents[1] / "config" / "mass-color-filter.txt") as file:
...
lowerBoundsRef = []
upperBoundsRef = []
with open(Path(__file__).resolve().parents[1] / "config" / "reference-color-filter.txt") as file:
    lines = file.readlines()
    for i in range(3, 6):
        tempList = lines[i].split(" ")
...
for fileName in os.listdir(videoFolderPath):
    filePath = str(videoFolderPath / fileName)
    ...
    with open(str(dataFolderPath / (fileName[:-10]+"-position-data.txt")), "w") as file:
